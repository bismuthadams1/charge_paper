...
def process_molecule(retrieved: MoleculePropRecord, conformer_no: int):
    ...
    #am1bcc chargeso
    am1bccmol = Molecule.from_mapped_smiles(mapped_smiles=retrieved.tagged_smiles, allow_undefined_stereo=True)
...
def process_and_write_batch(batch_models, schema, writer):
    """
    Sequentially process each (model, conformer_no) pair, writing out every 5 results as a RecordBatch.
    """
    results_batch = []
    # Loop sequentially with a progress bar
    batch_size = 1
    for model, conformer_no in tqdm(batch_models, desc='Processing molecules'):
        try:
            result = process_molecule(model, conformer_no)
            results_batch.append(result)

            # Once we have 5 results, write them out
            ...
            print(f'Failure processing ({model}, {conformer_no}) due to {e}')
            print(traceback.format_exc())
            # continue on errors
            continue
...
def main(output: str):
    ...
    with pyarrow.parquet.ParquetWriter(where=output, schema=schema, compression='snappy') as writer:
        ...
        for i in range(0, number_of_molecules, chunk_size):
            ...
            for batch in batched(batch_models, 1):
